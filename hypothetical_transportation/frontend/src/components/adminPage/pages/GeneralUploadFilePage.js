        ...
        if(userOK && studentOK){
            ...
            props.validate(jsonRes, () => {navigate("/upload_data")}, (err) => {
                if(err.response.status == 413){
                    setLoading(false);
                    alert("File(s) too large. Please split up any CSV files over 100MB and upload them separately.")
                }
            })
        }
        else{
            ...
            <Container className="container-main" style={{width: "50%"}} >
                {!loading?<Form className="shadow-lg p-3 mb-5 bg-white rounded"  noValidate onSubmit={handleOnSubmit}>
                    ...
                    {warning? <div>
                        ...
                        <p>Student CSV is expected to have "name", "parent_email", "student_id", "school_name" fields, in this order. If a student account should be created for the student,  you must also have a "student_email" field and optional "phone_number" field.</p>
