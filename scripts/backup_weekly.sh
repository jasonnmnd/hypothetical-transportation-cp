...
sudo docker cp ece-458_web_1:weekly.json weekly-$WEEK.json

echo "[WEEKLY - $WEEK] copying data dump from dev server host to backup host"
scp -i ~/.ssh/id_rsa weekly-$WEEK.json vcm@vcm-25708.vm.duke.edu:backups/dev-host/weekly/weekly-$WEEK.json