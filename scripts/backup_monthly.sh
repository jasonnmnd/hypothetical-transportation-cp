...
sudo docker cp ece-458_web_1:monthly.json monthly-$MONTH.json

echo "[MONTHLY - $MONTH] copying data dump from dev server host to backup host"
scp -i ~/.ssh/id_rsa monthly-$MONTH.json vcm@vcm-25708.vm.duke.edu:backups/dev-host/monthly/monthly-$MONTH.json